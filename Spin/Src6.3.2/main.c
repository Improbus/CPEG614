...
alldone(int estatus)
{
	...
	{	char *tmp;
		...
		} else if ((tmp = strchr(PreProc, ' ')) != NULL)
...
{	char precmd[1024], cmd[2048]; int i;
	...
	/* gcc is often a symbolic link to a specific version of gcc
	   that does not work well in Windows, so we find the actual
	   executable that is likely used:
	 */
...
usage(void)
{
	...
	printf("\t-run  compile and run\n");
	printf("\t      options following -run are passed to cc and pan\n");
	printf("\t      options before -run are interpreted by spin itself\n");
...
{	Symbol *s;
	...
	{	switch (argv[1][1]) {
		...
		case 'r':
			  ...
			  {	if (buzzed != 0)
				{ fatal("cannot combine -x and -run", (char *)0);
				}
				buzzed = 2;
				analyze = 1;
				argc--; argv++;
